...
class ServoNode(Node):
    def __init__(self) -> None:
        ...
        self.declare_parameter('bearing_deadband_deg', 1.5)
        self.declare_parameter('detection_timeout_s', 1.5)
        self.declare_parameter('obstacle_check_enabled', True)
        self.declare_parameter('obstacle_halt_distance_m', 1.2)
        ...
        self._pub_cmd_vel = self.create_publisher(Twist, '/cmd_vel_servo', 10)
        self.create_subscription(String, '/aruco/detection', self._detection_cb, 10)
        ...
        self.get_logger().info('Visual servo node started (inactive until START_SERVO).')
    ...
    def _detection_cb(self, msg: String) -> None:
        try:
            ...
            self.get_logger().warn('Invalid JSON on /aruco/detection')
            return

        if 'bearing_deg' not in data or 'distance_m' not in data:
            self.get_logger().warn('Detection JSON missing bearing_deg or distance_m')
            return

        self._latest_detection = data
    ...
    def _mission_cb(self, msg: String) -> None:
        ...
        if cmd == 'START_SERVO':
            self._active = True
            self._reset_pid()
            self.get_logger().info('Visual servo activated.')
        elif cmd == 'STOP_SERVO':
            self._active = False
            self._reset_pid()
            self.get_logger().info('Visual servo deactivated.')
        ...
        if self._detection_is_stale():
            if not self._detection_stale_warned:
                self.get_logger().warn(
                    f'No ArUco detection for >{self._detection_timeout_s():.1f}s — '
    ...
    def _compute_angular(self, bearing_deg: float) -> float:
        ...
        # Steer toward zero bearing: positive bearing (marker right) → turn right.
        error = -bearing_deg
        ...
        # Anti-windup: clamp integrator so I-term cannot exceed output limits alone.
        if self._ki() > 0.0:
    ...
    def _compute_linear(self, bearing_deg: float, distance_m: float) -> float:
        ...
        if distance_m < NEAR_RAMP_START_M:
            ...
            if distance_m > FAR_DISTANCE_M:
                ...
                # Linear ramp between 2.5 m (0.15 m/s) and 5.0 m (max_linear).
